...
/**
 * Implementation for physics layout is found here, since we want to offer the main layouts without
 * requiring further extension (LinearLayout, RelativeLayout, etc.)
 */
@Suppress("unused", "MemberVisibilityCanBePrivate")
class Physics
@JvmOverloads
constructor(private val viewGroup: ViewGroup, attrs: AttributeSet? = null) {
  ...
  companion object {
    ...
    /**
     * Set the configuration that will be used when creating the view Body. Changing view's
     * configuration after layout has been performed will require you to call
     * [ViewGroup.requestLayout] so that the body can be created with the new configuration.
     *
  ...
  /** Set the number of velocity iterations the world will perform at each step. Default is 8 */
  var velocityIterations = 8

  /** Set the number of position iterations the world will perform at each step. Default is 3 */
  var positionIterations = 3

  /**
   * Set the number of pixels per meter. Basically makes the world feel bigger or smaller Default is
   * 20dp. More pixels per meter = ui feeling bigger in the world (faster movement)
   */
  ...
  init {
    ...
    if (attrs != null) {
      val a = viewGroup.context.obtainStyledAttributes(attrs, R.styleable.Physics)
      ...
      pixelsPerMeter =
        a.getFloat(
          R.styleable.Physics_pixelsPerMeter,
          viewGroup.resources.getDimensionPixelSize(R.dimen.physics_layout_dp_per_meter).toFloat()
        )
      a.recycle()
    }
  }
  ...
  /**
   * Call this every time your view gets a call to onSizeChanged so that the world can respond to
   * this change.
   */
  ...
  /**
   * Call this every time your view gets a call to onLayout so that the world can respond to this
   * change.
   */
   ...
   * PhysicsConfigs and create a body in the world. This will override the current world if it
   * exists.
   */
  ...
  private fun createBody(view: View, oldBody: Body?): Body {
    ...
    fixtureDef.shape =
      if (config.shape == Shape.RECTANGLE) createBoxShape(view) else createCircleShape(view, config)
  ...
  /**
   * Finds the physics [Body] that corresponds to the view. Requires the view to have an id. Returns
   * null if no body exists for the view
   *
  ...
  fun giveRandomImpulse() {
    ...
    for (i in 0 until viewGroup.childCount) {
      impulse =
        Vec2((random.nextInt(1000) - 1000).toFloat(), (random.nextInt(1000) - 1000).toFloat())
  ...
  private fun translateBodyToView(body: Body, view: View) {
    body.setTransform(
      Vec2(pixelsToMeters(view.x + view.width / 2), pixelsToMeters(view.y + view.height / 2)),
      body.angle
    )
  }
  ...
  /**
   * Sets the gravity for the world. Positive x is right, negative is left. Positive y is down,
   * negative is up.
   */
  ...
  /**
   * Interface that allows hooks into the layout so that you can process or modify physics bodies
   * each time that JBox2D processes physics
   */
  interface OnPhysicsProcessedListener {

    /**
     * Physics has been processed. Commence doing things that you want to do such as applying
     * additional forces
     *
  ...
  interface OnCollisionListener {
     ...
     * R.id.physics_layout_bound_top, R.id.physics_layout_bound_bottom,
     * R.id.physics_layout_bound_left, or R.id.physics_layout_bound_right. If view was not assigned
     * an id, the return value will be [View.NO_ID].
     *
